...
from utils.utility_functions import enable_buttons, clear_frame, ui_font
...
def create_login_window(frame_main, buttons):
    ...
    tk.Label(frame_main, font=ui_font(), text="管理員帳號：").place(x=500, y=150)
    ...
    tk.Label(frame_main,font=ui_font(), text="管理員密碼：").place(x=500, y=200)
    ...
    button_login = tk.Button(frame_main,font=ui_font(), text="登入", width=15, command=on_login_clicked)
    button_login.place(x=600, y=250)

    label_register = tk.Label(frame_main,font=ui_font(), text="如果沒有管理員帳號，請先點擊下方按鈕註冊帳號")
    label_register.place(x=600, y=320)

    button_register = tk.Button(frame_main,font=ui_font(), text="註冊", width=15, command=lambda: create_register_window(frame_main, buttons))
