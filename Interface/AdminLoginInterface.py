# 管理員登入介面 AdminLoginInterface
import tkinter as tk
from tkinter import messagebox
from utils.utility_functions import *
from db.database import validate_admin_login
from Interface.start_window import start_window


def AdminLoginInterface(frame_main, buttons):
    clear_frame(frame_main)
    from .AdminRegistrationInterface import AdminRegistrationInterface
    
    # frmae title
    frame_title = frame_fun(frame_main)
    frame_title.pack(fill='x')
    label_fun(frame_title, '駕訓班管理員登入').pack(side='left', padx=20, pady=7)
    
    tk.Label(frame_main, font=custom_font(), fg='#626262', text="管理員帳號：").place(x=500, y=150)
    ...
    tk.Label(frame_main,font=custom_font(), text="管理員密碼：", fg='#626262').place(x=500, y=200)
    ...
    button_login = tk.Button(frame_main, fg='#626262', font=custom_font(), text="登入", width=15, command=on_login_clicked)
    button_login.place(x=600, y=250)

    label_register = tk.Label(frame_main, fg='#626262', font=custom_font(), text="如果沒有管理員帳號，請先點擊下方按鈕註冊帳號")
    label_register.place(x=600, y=320)

    button_register = tk.Button(frame_main, fg='#626262', font=custom_font(), text="註冊", width=15, command=lambda: AdminRegistrationInterface(frame_main, buttons))
